...
export default function Cadastro() {
  ...
  const formEmpregado = useForm<CadastroEmpregado>();
...
"x-api-key": "chave-primaria", },
  ...
  const aoEnviarEmpregado = async (data: CadastroEmpregado) => {
    try {
      const resposta = await fetch("https://eco-nomy-sis-stable-524v.onrender.com/empregado", {
        method: "POST",
        headers: { "Content-Type": "application/json","x-api-key": "chave-primaria", },
        body: JSON.stringify(data),
      });
      ...
      <div className="w-full max-w-md bg-[var(--c-bg6)] p-6 rounded-lg shadow-md">
        ...
        {tipoSelecionado === "empresa" && (
          <form onSubmit={formEmpresa.handleSubmit(aoEnviarEmpresa)} className="space-y-4">
            <input type="hidden" value="empresa" {...formEmpresa.register("tipo")} />
            ...
            <div>
              ...
              <input
                type="text"
                {...formEmpresa.register("cnpj", { required: "CNPJ é obrigatório." })}
                className="w-full px-4 py-2 rounded-md border"
              />
        ...
        {tipoSelecionado === "empregado" && (
          <form onSubmit={formEmpregado.handleSubmit(aoEnviarEmpregado)} className="space-y-4">
            <input type="hidden" value="empregado" {...formEmpregado.register("tipo")} />
            ...
            <div>
              ...
              <input
                type="text"
                {...formEmpregado.register("cpf", { required: "CPF é obrigatório." })}
                className="w-full px-4 py-2 rounded-md border"
              />
            ...
            <div>
              ...
              <input
                type="email"
                {...formEmpregado.register("email", { required: "Email é obrigatório." })}
                className="w-full px-4 py-2 rounded-md border"
              />
